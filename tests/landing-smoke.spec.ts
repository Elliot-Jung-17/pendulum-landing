...
test('mobile launch CTA stays inside the viewport', async ({ page }) => {
  ...
  expect(overflow.amount, JSON.stringify(overflow.offenders, null, 2)).toBeLessThanOrEqual(0);
});
...
test('capture mode freezes motion and produces a repeatable hero frame', async ({ page }, testInfo) => {
  ...
  async function capture() {
    ...
    await page.waitForFunction(() => /\d/.test(document.querySelector('[data-evidence="tests.passLabel"]')?.textContent ?? ''));
